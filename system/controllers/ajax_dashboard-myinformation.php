<?php 
	...
	if ($errored == 1) {
		...
		if ($action == 1) {	
			$fname_prev = $this->user->info->firstname;
			$lname_prev = $this->user->info->lastname;
			
			$dbcad = '';
			if ($fname_prev != $firstname || $lname_prev != $lastname) $dbcad = 'validated=0,';
			
			$this->db1->query("UPDATE users SET ".$dbcad." firstname='".$firstname."', lastname='".$lastname."', gender=".$gender.", born='".$born."' WHERE iduser=".$this->user->id.' LIMIT 1');
		...
		if ($action == 3) {	
			if ($didchanges == 1) {
				...
				if (file_exists($raiz.$tmppath.$loadedimage)) {
					$thumbnail = new SmartImage($raiz.$tmppath.$loadedimage, true);

					$thumbnail->myresize($C->widthAvatar0, 'width');
					$thumbnail->saveImage($raiz.$avatarfolder.$finalphoto);
					$thumbnail->close();
					
					$thumbnail = new SmartImage($raiz.$tmppath.$loadedimage, true);
					$thumbnail->mycrop($C->widthAvatar1, $C->heightAvatar1);
					$thumbnail->saveImage($raiz.$avatarfolder.'min1/'.$finalphoto);				
					$thumbnail->close();

					$thumbnail = new SmartImage($raiz.$tmppath.$loadedimage, true);
					$thumbnail->mycrop($C->widthAvatar2, $C->heightAvatar2);
					$thumbnail->saveImage($raiz.$avatarfolder.'min2/'.$finalphoto);
					$thumbnail->close();

					$thumbnail = new SmartImage($raiz.$tmppath.$loadedimage, true);
					$thumbnail->mycrop($C->widthAvatar3, $C->heightAvatar3);
					$thumbnail->saveImage($raiz.$avatarfolder.'min3/'.$finalphoto);
					$thumbnail->close();

					$thumbnail = new SmartImage($raiz.$tmppath.$loadedimage, true);
					$thumbnail->mycrop($C->widthAvatar4, $C->heightAvatar4);
					$thumbnail->saveImage($raiz.$avatarfolder.'min4/'.$finalphoto);
					$thumbnail->close();
					
					copy($raiz.$tmppath.$loadedimage, '../'.$C->FOLDER_AVATAR.'origins/'.$finalphoto);
		...
		if ($action == 7) {	
			if ($didchanges == 1) {
				...
				$this->db1->query("UPDATE users SET cover='".$finalphoto."' WHERE iduser=".$this->user->id.' LIMIT 1');
				
				$raiz = '../';
				$tmppath = $C->FOLDER_TMP;
				$coversfolder = $C->FOLDER_COVERS;
								
				// move photo file
				if (file_exists($raiz.$tmppath.$loadedimage)) {
					
					$thumbnail = new SmartImage($raiz.$tmppath.$loadedimage, true);
					$thumbnail->mycrop($C->widthCover1, $C->heightCover1);
					$thumbnail->saveImage($raiz.$coversfolder.'min1/'.$finalphoto);				
					$thumbnail->close();

					$thumbnail = new SmartImage($raiz.$tmppath.$loadedimage, true);
					$thumbnail->mycrop($C->widthCover2, $C->heightCover2);
					$thumbnail->saveImage($raiz.$coversfolder.'min2/'.$finalphoto);				
					$thumbnail->close();

					$thumbnail = new SmartImage($raiz.$tmppath.$loadedimage, true);
					$thumbnail->mycrop($C->widthCover3, $C->heightCover3);
					$thumbnail->saveImage($raiz.$coversfolder.'min3/'.$finalphoto);				
					$thumbnail->close();
					
					copy($raiz.$tmppath.$loadedimage, '../'.$C->FOLDER_COVERS.$finalphoto);
					
					unlink($raiz.$tmppath.$loadedimage);
			
					$txtreturn = $this->lang('dashboard_mi_mcov_form_ok');
					echo("1: ".$txtreturn);
					return;
				} else {
