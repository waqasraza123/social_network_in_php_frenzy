<?php
	...
	if ($errored == 1) {
		...
		$totalitems = $this->db2->fetch_field('SELECT count(iduser) FROM users, relations WHERE iduser=subscriber AND leader='.$iduser);
		
		$r = $this->db2->query('SELECT iduser, code, firstname, lastname, username, avatar, num_posts, num_followers, num_following, validated FROM users, relations WHERE iduser=subscriber AND leader='.$iduser.' ORDER BY rltdate DESC LIMIT '.$numitems.','.$C->NUM_FOLLOWERS_PAGE);
		...
		while( $obj = $db2->fetch_object($r) ) {
			...
			$htmlResults .= $this->load_template('__profile-one-follower.php', FALSE);
		}
