<?php
	...
	if ($errored == 1) {
		...
		$totalitems = $this->db2->fetch_field('SELECT count(iduser) FROM users, relations WHERE iduser=leader AND subscriber='.$this->user->id);
		
		$r = $this->db2->query('SELECT iduser, code, firstname, lastname, username, avatar, num_posts, num_followers, num_following, validated FROM users, relations WHERE iduser=leader AND subscriber='.$this->user->id.' ORDER BY rltdate DESC LIMIT '.$numitems.','.$C->NUM_FOLLOWING_PAGE);
		...
		while( $obj = $db2->fetch_object($r) ) {
			...
			$htmlResults .= $this->load_template('__dashboard-one-following.php', FALSE);
		}
