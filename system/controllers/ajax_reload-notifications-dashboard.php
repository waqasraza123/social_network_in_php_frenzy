<?php
	...
	$r = $this->db2->query('SELECT notif_type, notif_object_id, username, firstname, lastname, avatar, date, users.code as ucode, idresult FROM notifications, users WHERE users.iduser=from_user_id AND to_user_id='.$this->user->id.' ORDER BY date DESC LIMIT '.$numitems.', '.$C->NUM_NOTIFICATIONS_PAGE);
	...
	while( $obj = $this->db2->fetch_object($r) ) {
		...
		$htmlResults .= $this->load_template('__dashboard-one-notification.php', FALSE);
	}
