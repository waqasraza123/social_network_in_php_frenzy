<?php 
	...
	if ($errored == 1) {
		...
		$r = $this->db2->query("SELECT posts.*, username, firstname, lastname, avatar, users.iduser, users.code as ucode FROM posts, users WHERE typepost='photo' AND  posts.iduser=".$D->u->iduser." AND posts.iduser=users.iduser ORDER BY posts.whendate DESC LIMIT ".$numitems.",".$C->NUM_ACTIVITIES_PAGE);
