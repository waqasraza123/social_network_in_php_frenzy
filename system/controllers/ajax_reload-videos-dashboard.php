<?php
	...
	if ($errored == 1) {
		...
		//We load the posts
		$totalitems = $this->db2->fetch_field("SELECT count(idpost) FROM posts WHERE typepost='video' AND iduser=".$this->user->id);
		
		$r = $this->db2->query("SELECT posts.*, username, firstname, lastname, avatar, users.iduser, users.code as ucode FROM posts, users WHERE typepost='video' AND posts.iduser=".$this->user->id." AND posts.iduser=users.iduser ORDER BY posts.whendate DESC LIMIT ".$numitems.",".$C->NUM_ACTIVITIES_PAGE);
