<?php
	...
	$D->totalposts = $this->db2->fetch_field("SELECT count(idpost) FROM posts WHERE typepost='photo' AND iduser=".$this->user->id);
	
	$r = $this->db2->query("SELECT posts.*, username, firstname, lastname, avatar, users.iduser, users.code as ucode FROM posts, users WHERE typepost='photo' AND  posts.iduser=".$this->user->id." AND posts.iduser=users.iduser ORDER BY posts.whendate DESC LIMIT 0,".$C->NUM_ACTIVITIES_PAGE);
	...
	$D->optionactive = 3;
	$this->load_template('dashboard-myphotos.php');
?>
