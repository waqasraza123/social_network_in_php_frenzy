...
if ($errored == 1) {
	...
	$sizeimage = $_FILES['uploaded_cover']['size'];
	$loadedtype = $_FILES['uploaded_cover']['type'];
	...
	if ($loadedtype=="image/jpeg" || $loadedtype=="image/gif" || $loadedtype=="image/png") {
		...
		if ($sizeimage<$C->SIZE_IMAGEN_COVER) {
			 // move the file to the specified folder
			 if (move_uploaded_file($_FILES['uploaded_cover']['tmp_name'], $uploadfile)) {
				echo('1: '.$mfilename);
				return;
			 } else {
				echo('0: '.$this->lang('dashboard_mi_mcov_form_msg7'));
				return;
			 }
		} else {
			echo('0: '.$this->lang('dashboard_mi_mcov_form_msg6'));
			return;
		} 
	} else { 
		echo('0: '.$this->lang('dashboard_mi_mcov_form_msg8'));
		return;
	}	
	echo("1: Ok");
	return;
