<?php
	...
	if ($D->show_profile==1) {
		...
		$D->totalposts = $this->db2->fetch_field("SELECT count(idpost) FROM posts WHERE typepost='video' AND iduser=".$D->u->iduser);
		
		$r = $this->db2->query("SELECT posts.*, username, firstname, lastname, avatar, users.iduser, users.code as ucode FROM posts, users WHERE typepost='video' AND  posts.iduser=".$D->u->iduser." AND posts.iduser=users.iduser ORDER BY posts.whendate DESC LIMIT 0,".$C->NUM_ACTIVITIES_PAGE);
