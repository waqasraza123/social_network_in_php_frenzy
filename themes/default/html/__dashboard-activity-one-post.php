        ...
        <?php if (!empty($D->typepost)) {
				...
				if ($D->typepost=='music') {
					...
					if(substr($D->valueattach, 0, 3) == 'sc:') {
						$cadattach .= '<div style="padding:0px 10px 10px 0px;"><iframe width="100%" height="'.$height.'" scrolling="no" frameborder="no" src="https://w.soundcloud.com/player/?url=https://soundcloud.com'.str_replace('sc:', '', $D->valueattach).'"></iframe></div>';
					} else {
						$cadattach .= '<div style="padding:0px 0px 10px;"><div><img src="'.$C->SITE_URL.'themes/'.$C->THEME.'/imgs/icomusic.png"></div><div style="font-size:13px; color:#999">'.$this->lang('global_post_txt_ilistened').'</div><div style="font-size:16px; font-weight:bold; word-wrap: break-word;">'.$D->valueattach.'</div></div>';
