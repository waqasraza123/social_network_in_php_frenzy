...
function updateMyCover(diverror, divok, bsumbit)
{
	$(bsumbit).attr('disabled','true');

	loadedcover=$.trim($('#loadedcover').val());
	if (loadedcover == '') {
		openandclose(diverror,mi_mcov_msg3,1700);
		setTimeout(function() {$(bsumbit).removeAttr('disabled');}, 2000); 
		return;
	}
	
	didchangescov=$('#didchangescov').val();

	$.ajax({
		type: 'POST',
		url: siteurl + 'ajax/dashboard-myinformation/r:' + Math.round(Math.random()*1000),
		data: 'todo=7&dch=' + didchangescov + '&limg=' + loadedcover,
		success: function(resp){
			switch(resp.charAt(0)){
				case '0':
					openandclose(diverror,resp.substring(3),2000);
					...
					break;
				case '1':
					$('#loadedcover').val(loadedimage);
					$('#didchangescov').val(0);
					oneimage = new Image();
					oneimage.src = siteurl + foldercovers + loadedcover;
					openandclose(divok,resp.substring(3),2000);
					setTimeout(function() {$(bsumbit).removeAttr('disabled');}, 3000); 
					break;
			}
