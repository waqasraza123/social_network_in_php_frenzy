function reloadinfo(thatthing)
{
	...
	switch (thatthing) {
		...
		case 'following':
			...
			break;
		case 'videos':
			urltarget = 'ajax/reload-videos/r:';
			break;
		case 'photos':
			urltarget = 'ajax/reload-photos/r:';
			break;
		case 'likes':
			...
			switch(resp.charAt(0)){
				...
				case '1':
					if (whatdo==1) {
						...
						$('.liked').html('<span><img src="' + siteurl + 'themes/' + sitetheme + '/imgs/icolike.png"></span>');
					} else {
						if (whatdo==0) {
							$('#numlikes').html(parseInt($('#numlikes').html()) - 1);
							$('.liked').html('<span><img src="' + siteurl + 'themes/' + sitetheme + '/imgs/icounlike.png"></span>');
						}						
					}
					break;
			};				
		...
		success: function(resp){
			switch(resp.charAt(0)){
				...
				case '1':
					if (whatdo==1) {
						//$('#numlikes').html(parseInt($('#numlikes').html()) + 1);
						$('.censored').html('<span><img src="' + siteurl + 'themes/' + sitetheme + '/imgs/icoflagred.png"></span>');
					} else {
						if (whatdo==0) {
							//$('#numlikes').html(parseInt($('#numlikes').html()) - 1);
							$('.censored').html('<span><img src="' + siteurl + 'themes/' + sitetheme + '/imgs/icoflaggreen.png"></span>');
						}						
					}
					break;
			};				
